...
export default function PropertiesPanel() {
    ...
    return (
        <div className="absolute z-50 right-6 top-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-slate-200/50 w-14 flex flex-col items-center py-3 gap-2 transition-all">
            <div className="relative">
                ...
                {activeMenu === 'opacity' && (
                    <div className="absolute right-14 top-0 bg-white rounded-xl shadow-xl border border-slate-200 p-3 w-48 flex flex-col gap-2">
            ...
            <div className="relative">
                ...
                {activeMenu === 'bg' && (
                    <div className="absolute right-14 top-0 bg-white rounded-xl shadow-xl border border-slate-200 p-3 w-40 flex flex-col gap-3">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Pattern</span>
                        <div className="flex bg-slate-100 p-1 rounded-lg">
                            {['solid', 'dotted', 'grid'].map((pat) => (
                                <button
                                    key={pat}
                                    onClick={() => setBgPattern(pat as 'solid' | 'dotted' | 'grid')}
                                    className={`flex-1 py-1 text-[10px] font-bold rounded capitalize transition-all ${bgPattern === pat ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-800'}`}
                                >
                                    {pat}
                                </button>
                            ))}
