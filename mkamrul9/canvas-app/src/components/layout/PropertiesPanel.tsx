...
export default function PropertiesPanel() {
    const { activeColor, setActiveColor, bgMode, setBgMode, customColors, addCustomColor } = useCanvasStore();
    ...
    return (
        <div className="absolute z-50 right-6 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-200/50 w-16 flex flex-col items-center py-4 gap-3 transition-all">
            ...
            <label
                className="relative w-9 h-9 rounded-full cursor-pointer flex items-center justify-center shadow-sm hover:scale-105 transition-transform"
            ...
            >
                ...
                <input
                    ...
                    onInput={(event) => setActiveColor((event.target as HTMLInputElement).value)}
                    onBlur={(event) => addCustomColor((event.target as HTMLInputElement).value)}
                />
            </label>

            <div className="flex flex-col gap-2 mt-1">
                {[...customColors, ...DEFAULT_COLORS].slice(0, 9).map((color, index) => (
                    <button
                        key={`${color}-${index}`}
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center leading-tight mb-1">
                BG
                <br />
                Mode
            </span>
            <button
                ...
                className="w-9 h-9 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 transition-colors flex items-center justify-center shadow-sm"
                title="Cycle Background (Dotted, White, Black)"
            >
                <div
                    className={`w-5 h-5 rounded-sm ${bgMode === 'white'
                        ? 'bg-white border-2 border-slate-300'
                        : bgMode === 'black'
                            ? 'bg-slate-900 border-2 border-slate-700'
                            : 'bg-slate-100 border-2 border-dashed border-slate-400'
                        }`}
                ></div>
            </button>
        </div>
