            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {visibleBoards.map((board) => (
                    <Link key={board.id} href={`/board/${board.id}`} className="group block h-full">
                        <div className="bg-white/[0.03] rounded-3xl border border-white/10 shadow-2xl overflow-hidden hover:shadow-[0_0_50px_rgba(139,92,246,0.15)] hover:border-violet-500/50 hover:bg-white/[0.05] transition-all duration-500 h-full flex flex-col group-hover:-translate-y-2 backdrop-blur-xl">
                            <div className="h-44 bg-[#030407] border-b border-white/5 flex items-center justify-center relative overflow-hidden">
