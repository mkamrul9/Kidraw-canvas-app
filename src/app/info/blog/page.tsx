...
import { ArrowLeft, ArrowUpRight, Calendar, User, LayoutGrid, Cpu, PenTool, Sparkles, Rocket, Clock, ChevronRight } from "lucide-react";
        ...
        <div className="min-h-screen bg-background text-foreground relative overflow-hidden font-sans selection:bg-amber-500/30">
            ...
            <main className="flex-1 max-w-7xl w-full mx-auto p-8 py-20 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                    <div>
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary/50 border border-border text-xs font-bold text-amber-400 mb-6 shadow-sm">
                            <PenTool className="w-3 h-3" /> The Kidraw Blog
                        </div>
                        <h1 className="text-5xl font-bold text-foreground tracking-tight">Updates & Insights</h1>
                    </div>
                    <div className="flex gap-2">
                        {['All', 'Engineering', 'Product', 'Design'].map((tag, i) => (
                            <button key={i} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${i === 0 ? 'bg-white text-black' : 'bg-card border border-border text-muted-foreground hover:text-foreground hover:bg-accent'}`}>
                                {tag}
                            </button>
                        ))}
                    </div>
                ...
                {/* Featured Post */}
                <div className="group cursor-pointer mb-20 relative overflow-hidden rounded-[2rem] border border-border bg-card/50 backdrop-blur-sm transition-all hover:border-white/30 flex flex-col md:flex-row min-h-[400px]">
                    ...
                    <div className="w-full md:w-1/2 p-10 md:p-16 flex flex-col justify-center relative">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <span className="text-xs text-muted-foreground flex items-center gap-1"><Calendar className="w-3 h-3"/> Oct 24, 2024</span>
                        </div>
                        <h2 className="text-4xl font-bold text-foreground mb-6 group-hover:text-amber-400 transition-colors">
                        ...
                        <div className="flex items-center justify-between mt-auto">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-foreground">Alex Designer</p>
                                    <p className="text-xs text-muted-foreground">Head of Design</p>
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
                    {[
                        ...
                        <div key={i} className="group cursor-pointer rounded-3xl border border-border bg-card/50 backdrop-blur-sm overflow-hidden hover:border-white/30 transition-all flex flex-col">
                            ...
                            <div className="p-8 flex flex-col flex-1">
                                ...
                                <h3 className="text-xl font-bold text-foreground mb-3 group-hover:text-amber-400 transition-colors line-clamp-2">{post.title}</h3>
                                <p className="text-muted-foreground text-sm leading-relaxed mb-6 line-clamp-2">{post.desc}</p>
                                <div className="mt-auto flex items-center justify-between">
                                    <span className="text-xs font-medium text-muted-foreground flex items-center gap-1"><User className="w-3 h-3"/> Eng Team</span>
                                    <ArrowUpRight className="w-4 h-4 text-zinc-600 group-hover:text-foreground transition-colors" />
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                {/* Newsletter */}
                <div className="bg-card border border-border rounded-[2rem] p-12 text-center max-w-3xl mx-auto backdrop-blur-sm shadow-2xl relative overflow-hidden">
                    ...
                    <h2 className="text-3xl font-bold text-foreground mb-4 relative z-10">Subscribe to our newsletter</h2>
                    <p className="text-muted-foreground mb-8 relative z-10">Get the latest product updates and engineering deep-dives straight to your inbox. No spam, ever.</p>
                    <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto relative z-10">
                        <input type="email" placeholder="Email address..." className="flex-1 bg-card border border-border rounded-xl px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-amber-500/50" />
