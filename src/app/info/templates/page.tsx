...
export default function TemplatesPage() {
    return (
        <div className="min-h-screen bg-black text-slate-50 relative overflow-hidden font-sans selection:bg-fuchsia-500/30">
            {/* Background Layers */}
            ...
            <main className="flex-1 max-w-7xl w-full mx-auto p-8 py-20 relative z-10">
                ...
                <div className="text-center mb-20 relative">
                    ...
                    <div className="max-w-xl mx-auto relative group">
                        <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                            <Search className="w-5 h-5 text-zinc-500 group-focus-within:text-fuchsia-400 transition-colors" />
                        </div>
                        <input 
                            type="text" 
                            placeholder="Search templates (e.g., 'Wireframe', 'AWS Architecture')..." 
                            className="w-full bg-zinc-900/50 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-fuchsia-500/50 backdrop-blur-sm transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="flex flex-wrap justify-center gap-3 mb-16">
                    {['All Templates', 'Wireframes', 'Architecture', 'Mind Maps', 'Agile / Scrum', 'User Journeys'].map((category, i) => (
                        <button key={i} className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all ${i === 0 ? 'bg-white text-black shadow-[0_0_20px_rgba(255,255,255,0.2)]' : 'bg-zinc-900/50 border border-white/10 text-zinc-400 hover:text-white hover:bg-zinc-800'}`}>
                            {category}
                        </button>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {[
                        ...
                        <div key={i} className="group cursor-pointer rounded-3xl border border-white/5 bg-zinc-900/40 backdrop-blur-sm overflow-hidden hover:border-white/20 transition-all flex flex-col">
                            {/* Mockup Preview Area */}
                            <div className={`h-40 w-full bg-gradient-to-br ${template.bg} relative flex items-center justify-center overflow-hidden`}>
                                ...
                                <div className="text-white/30 group-hover:scale-110 group-hover:text-white/50 transition-all duration-500">
                                    {template.icon}
                                </div>
                            ...
                            <div className="p-5 flex flex-col flex-1">
                                ...
                                <h3 className="text-lg font-bold text-white group-hover:text-fuchsia-300 transition-colors">{template.title}</h3>
                            </div>
                        </div>
                    ))}
                </div>
